...
def get_api_keys(
    ...
    Tavily is NOT BYOK: it's a server-side-only key (see app/config/server.py
    and app/service/graphRunner.py), since Tavily's free tier (1000
    credits/month) comfortably covers the web-search fallback path without
    asking every user for their own key.
    """
