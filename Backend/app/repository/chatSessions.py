...
async def ensure_indexes() -> None:
    ...
    await collection.create_index([("session_id", 1), ("updated_at", -1)])
    await collection.create_index("thread_id", unique=True)


async def get_thread(thread_id: str) -> Optional[dict]:
    return await _collection().find_one({"thread_id": thread_id})
...
async def list_threads(session_id: str) -> list[dict]:
...
async def get_thread_owner(thread_id: str) -> Optional[str]:
    doc = await _collection().find_one({"thread_id": thread_id}, {"session_id": 1})
